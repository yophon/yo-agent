...
｜ 🚧 **Phase 4 进行中**（bash 工具集补全 + L1 子进程沙箱 + 子 agent + 插件隔离 —— 开放渠道前的安全底座，6 片 4A-4F；L2 容器/OTel 顺延 Phase 6）：**4A 横切底座已交付**（生命周期 Hook 矩阵 + permissionMode→PolicyEngine 闸门 + ExecBackend 抽象，无运行时行为变更，+28 测试 → **335**）—— 见 [`docs/PHASE-4.md`](docs/PHASE-4.md)。
