...
> **范围**：单进程 self-host 起步，所有边界按"一个内核 × 多 surface × 多 provider × 多平台"建模。yo-agent 是 **TypeScript/Node 单栈通用 agent 引擎**：同一个内核既能当编程 agent（读写代码 / 跑命令 / diff 审批，对标 Claude Code / Codex / opencode / pi），又能挂接 QQ / Telegram / Discord 等聊天平台（对标 AstrBot / nanobot / openclaw），并且能被 **yo-aichat 的 Go agent-bridge 当作"自研第四类 agent"** 用 cursor-可恢复的 JSON-RPC/JSONL 协议驱动（对标 codex app-server / pi `--mode rpc`）。
>
...
>
> **三句话定全局**：(a) `Provider` 把 Anthropic / OpenAI / Gemini / DeepSeek / 任意 OpenAI 兼容端点归一成一条 `Stream<ProviderEvent>` + 双轨工具调用（native function-calling 优先，prompt-and-parse 回退）；(b) `Kernel` 把"组装上下文 → 调 provider → 执行工具 → 审批 → 注入 observation → 熔断/压缩/恢复"归一成一条 append-only `Stream<AgentEvent>`，落盘 SQLite 事件日志，支持 `resume(cursor)`；(c) `Surface`（CLI / 聊天平台 / IDE-via-ACP / RPC-for-bridge）只消费同一条 `AgentEvent` 流并回灌输入，**永不按内核内部分支**。
...
2. **可恢复协议一等公民**：从第一天就说一套与 yo-aichat `AgentEvent` 同构的 sealed 事件流 + JSON-RPC 方法集，支持 `resume(cursor)`、`turn/steer`、`turn/interrupt`、协议级审批。**这套协议同时满足"被 Go bridge 当第四类 agent 驱动"和"独立 CLI / 挂 IM"**（借鉴 codex app-server 三层事件流 + pi `--mode rpc`）。
...
6. **安全可控**：多档权限 + 协议化审批 + 工具沙箱 + 危险命令防护 + 注入防护 + 复用 yo-aichat 的 ed25519 + 配对码设备鉴权。
...
| **Protocol** | 事件/方法单一事实源；与 yo-aichat AgentEvent 同构；schema-gen 出 TS + Go。 | 借鉴 codex app-server Thread/Turn/Item、pi `--mode rpc`、ACP。 |
...
**三层概念**（直接对齐 codex app-server 的 Thread/Turn/Item，便于 bridge 对接）：
...
与 yo-aichat 的 `AgentEvent` sealed union **逐项同构**（见 yo-aichat DESIGN §5.4），使 Go bridge 能像归一 codex/claude/pty 一样零成本归一 yo-agent。

```typescript
// protocol/src/events.ts —— 单一事实源（同时 schema-gen 出 JSON Schema 给 Go bridge）
type Cursor = number;          // bridge 分配 / 本地单调递增
type Id = string;              // ULID
...
**审批（与沙箱正交，借鉴 Codex「审批策略 × 沙箱模式独立配置」）**：见 §9.2。审批走 **ACP 式协议消息**而非进程内 UI——`emit(ApprovalRequested{requestId, risk, suggestions})`，前端（CLI 弹窗 / IM 按钮 / IDE 对话框）渲染相同语义，`approval.decide(requestId, 'allow_once'|'allow_always'|'reject_once'|'reject_always', updatedInput?)` 回灌。这与 yo-aichat bridge 的原生审批回路天然对接。
...
### 4.2 五个 adapter（与 yo-aichat 完全对齐，复用其踩坑结论）
...
## 6. 会话与可恢复协议（与 yo-aichat bridge 对接）

> 这是 yo-agent 的**命脉章节**：yo-agent 要被 yo-aichat 的 Go bridge 当"第四类 agent"驱动，必须像 codex app-server 一样说一套持久、可恢复、协议级审批的 JSON-RPC。

### 6.1 协议形态决策

**双形态同一内核**（pi 三模态复用同一内核的范式）：

- **`yo-agent rpc`**（默认给 bridge / 嵌入式）：**JSON-RPC 2.0 over (TLS) socket**（stdio / Unix socket / WebSocket），事件用 `event` notification 推送。这是 bridge 的主路（对标 codex app-server）。
...
### 6.2 JSON-RPC 方法表（bridge → yo-agent）

直接以 **codex app-server 为蓝本**，并与 yo-aichat bridge 的 `session.*` RPC（DESIGN §5.3）逐项对齐，使 bridge 能像驱动 codex app-server 一样驱动 yo-agent：
...
| **S→C** | `approval/request(requestId, sessionId, tool, input, risk, suggestions)` | **bridge→app 主动请求审批，阻塞 agent 直到应答/超时（默认 deny）** | codex serverRequest/approval、yo-aichat `approval.request` |
| C↔S | `ping / pong` | 心跳 | 全员 |
...
```
bridge 重连 → session/resume(sessionId, fromCursor=last_seen):
  ├─ fromCursor 仍在内存 ResumeBuffer → 重放 [fromCursor+1 .. head] 缺口 + 续实时
  └─ fromCursor 被淘汰（gap 溢出）→ 从 SQLite 全量 EventLog 取窗口内
      "状态变更/审批/FileChanged" 摘要（审计不丢，yo-aichat replay_gap_overflow）
      + 当前快照，标注"中间过程已折叠"
```

**审批跨重连存活**（yo-aichat 关键 gap）：`approval/request` 进 ResumeBuffer，bridge 断线时审批 pending 不无限挂起——设超时（默认 5 分钟）**默认 deny**，重连时重放未决审批。

### 6.4 与 yo-aichat AgentEvent 同构性保证

yo-agent 的 `AgentEvent`（§2.2）**逐项映射** yo-aichat bridge 的 sealed `AgentEvent`（DESIGN §5.4）：`SessionStarted/AssistantText/Reasoning/ToolCallStarted/ToolCallOutput/ToolCallCompleted/FileChanged/Todo/ApprovalRequested/ApiRetry/TurnCompleted/TurnFailed/BackgroundProcess/Error` 一一对应。bridge 的 `CodexAdapter/ClaudeAdapter/PtyAdapter` 之外新增 **`YoAgentAdapter`**，只需把 yo-agent 的 JSON-RPC 事件转成 bridge 的归一事件——**因为同构，这层转换几乎是恒等映射**。鉴权复用 yo-aichat 的 ed25519 + 配对码 + nonce 挑战（§9）。
...
| **RpcSurface** | JSON-RPC 2.0 / JSONL，给 yo-aichat bridge & 嵌入式（§6） | codex app-server、pi rpc |
...
| **bridge 驱动** | 跟随 bridge 协议审批 | 跟随 opts | 审批走 §6.2 `approval/request`，bridge→app 渲染 |

至少 **read-only / supervised / autonomous 三档 PermissionMode**（横向共识下限），会话内可临时升级 + 时间窗。审批走**协议消息**（ACP four-option），前端无关。

### 9.3 设备鉴权（复用 yo-aichat）

RpcSurface（被 bridge 驱动）与开放渠道复用 yo-aichat 鉴权模型：
...
- 短 TTL token + 静默轮换 + 撤销实时断链（yo-aichat §8.7）。
...
| 鉴权 | **`@noble/ed25519`** + argon2id（token hash） | 复用 yo-aichat |
...
- `protocol/`：`AgentEvent` sealed union + JSON-RPC 方法表 + StopReason + cursor/resume 语义；TS 类型 + JSON Schema（可 gen Go 给 bridge）。
- pnpm workspace + `kernel`/`store`/`provider`/`tools` 接口冻结。
- 四接口冻结：`Provider` / `ToolDescriptor+ExecutorRef` / `Surface` / `Condenser`。
- **退出标准**：协议 schema 冻结，EventLog schema_version 入库，与 yo-aichat AgentEvent 同构性 review 通过。

### Phase 1 — 内核 + 编程 CLI MVP（第一可交付，零网络风险）
...
### Phase 2 — RPC 协议 + bridge 对接（核心命脉，第二能力）

- RpcSurface：JSON-RPC 2.0（stdio/Unix socket/WS）+ `--mode jsonl`；`session/* turn/* approval/*` 方法集。
- cursor resume + ResumeBuffer + gap 溢出降级 + 审批跨重连存活。
- auth：ed25519 + 配对码 + nonce 挑战（复用 yo-aichat）。
- **与 yo-aichat 联调**：bridge 新增 `YoAgentAdapter`，验证 bridge 能像驱动 codex app-server 一样驱动 yo-agent（断网/重启 resume 不丢 token/不丢审批）。
- **退出标准**：yo-aichat app 经 bridge 经隧道驱动 yo-agent，resume 四要素（externalId/workspace/gitRef/cursor）精确续接。

### Phase 3 — MCP 双向 + 聊天平台接入

- MCP host（懒加载+TTL+熔断）+ MCP server（`--mcp-server`）。
- ChatSurface：Transport+Adapter 二层 + OneBot v11（QQ）+ DM pairing；ConfirmationPolicy 切聊天态。
- ContextCompacted 结构化 Handoff + 不透明标识符保留；动态 auto-memory。
- **退出标准**：QQ 群驱动 yo-agent（审批/pairing/压缩跑通）；yo-agent 作 MCP server 被 Claude Code 调用。

### Phase 4 — 子 agent + 沙箱加固 + 可观测

- SubagentManager（Worker 隔离 + 异步 steering）+ recipes/skills 懒加载。
- L1 子进程隔离 + L2 容器（opt-in）+ 危险命令防护 + 注入防护三阶段审查。
- OTel 全链路 + 用量计费 + fallback 链/auth rotation。
- 插件 SDK（Worker IPC 隔离）+ hooks 矩阵。
- **退出标准**：安全审查通过；子 agent 崩溃不拖垮主循环；插件隔离生效。

### Phase 5 — ACP + 多平台打磨 + 多用户接缝

- AcpSurface（被 Zed/JetBrains 接管）；Telegram/Discord adapter。
- repo map（tree-sitter）；RAG 长期记忆（opt-in）。
- 多用户/团队接缝兑现（per-device→machine 授权矩阵，复用 yo-aichat §8.6 思路）。
...
| **与 bridge 协议偏移** | protocol 单一事实源 schema-gen TS+Go；CI 跨语言 schema 一致性测试 |
...
- **理由**：免费得到 resume + 确定性重放 + 审计三件套，直接解决"被 bridge 当第四类 agent 驱动需 resume(cursor)"与"IM 长会话恢复"两大刚需。
...
- **决策**：以 codex app-server 为蓝本设计 RPC（`session/* turn/* approval/* fs/* model/*`），与 yo-aichat bridge `session.*` 逐项对齐；同时提供 `--mode jsonl` 轻量形态。
- **理由**：bridge 已能驱动 codex app-server，同构协议使 `YoAgentAdapter` 几乎是恒等映射；三模态复用同一内核（pi）。
- **对比**：codex app-server（主范本，最成熟）vs pi `--mode rpc`（轻量范本，采纳为 jsonl 形态）vs ACP（采纳其 request_permission 四选项 + StopReason，但 ACP 远端传输 WIP 不作主路）。
- **被否**：自研全新协议（与 bridge 对接成本高）；纯 REST（无法双向 push 审批）。
